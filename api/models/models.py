...
auth = HTTPBasicAuth()
DATABASE_URL = 'postgresql+psycopg2://{user}:{pw}@{url}/{db}'.format(user=postgres,pw=postgres,url='jdbc:postgresql://localhost:5432/postgres',db=postgres)
...
class User(db.Model):
    ...
    is_active = db.Column('is_active', db.Boolean, default=True)
    creation_date = db.Column('creation_date', db.DateTime)
    last_update = db.Column('last_update', db.DateTime, cascade="All")
...
class Seller(db.Model):
    ...
    email_id = db.Column('email_id', db.Text, nullable=False)
    is_active = db.Column('is_active', db.Boolean, default=True)
    creation_date = db.Column('creation_date', db.DateTime)
    last_update = db.Column('last_update', db.DateTime)
